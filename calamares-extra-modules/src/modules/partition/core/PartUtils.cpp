...
QString convenienceName(const Partition *const candidate) {
  ...
  QString p;
  QTextStream s(&p);
  s << static_cast<const void *>(
      candidate); // No good name available, use pointer address

  return p;
}

/** @brief Get the globalStorage setting for required space. */
static double getRequiredStorageGiB(bool &ok) {
...
bool canBeReplaced(Partition *candidate, const Logger::Once &o) {
  ...
  cDebug() << o << "Checking if" << convenienceName(candidate)
           << "can be replaced.";
  ...
  if (!ok) {
    cDebug() << Logger::SubEntry
             << "NO, requiredStorageGiB is not set correctly.";
    return false;
  }

  qint64 availableStorageB = candidate->capacity();
  qint64 requiredStorageB =
      CalamaresUtils::GiBtoBytes(requiredStorageGiB + 0.5);

  if (availableStorageB > requiredStorageB) {
    cDebug() << o << "Partition" << convenienceName(candidate)
             << "authorized for replace install.";
    return true;
  } else {
    Logger::CDebug deb;
    ...
    deb << Logger::Continuation << "Available storage B:" << availableStorageB
        << QString("(%1GiB)").arg(
               CalamaresUtils::BytesToGiB(availableStorageB));
    return false;
  }
}
...
bool canBeResized(Partition *candidate, const Logger::Once &o) {
  ...
  if (!candidate->fileSystem().supportGrow() ||
    ...
    cDebug() << o << "Can not resize" << convenienceName(candidate)
             << ", filesystem" << candidate->fileSystem().name()
  ...
  if (candidate->roles().has(PartitionRole::Primary)) {
    ...
    if (table->numPrimaries() >= table->maxPrimaries()) {
      cDebug() << o << "Can not resize" << convenienceName(candidate)
               << ", partition table already has" << table->maxPrimaries()
               << "primary partitions.";
      return false;
    }
  }
  ...
  if (availableStorageB > advisedStorageB) {
    ...
    deb << Logger::Continuation << "Available storage B:" << availableStorageB
        ...
        << "for" << convenienceName(candidate)
        << "length:" << candidate->length()
        << "sectorsUsed:" << candidate->sectorsUsed()
        << "fsType:" << candidate->fileSystem().name();
    return false;
  }
}
...
bool canBeResized(DeviceModel *dm, const QString &partitionPath,
  ...
  if (partitionPath.startsWith("/dev/")) {
    for (int i = 0; i < dm->rowCount(); ++i) {
      ...
      Partition *candidate =
          CalamaresUtils::Partition::findPartitionByPath({dev}, partitionPath);
      if (candidate) {
    ...
    cWarning() << "Can not resize" << partitionPath
               << ", does not start with /dev";
    return false;
  }
}

static FstabEntryList lookForFstabEntries(const QString &partitionPath) {
  ...
  CalamaresUtils::Partition::TemporaryMount mount(partitionPath, QString(),
                                                  mountOptions.join(','));
  if (mount.isValid()) {
    QFile fstabFile(mount.path() + "/etc/fstab");

    if (fstabFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
      const QStringList fstabLines =
          QString::fromLocal8Bit(fstabFile.readAll()).split('\n');
      ...
      std::remove_if(fstabEntries.begin(), fstabEntries.end(),
                     [](const FstabEntry &x) { return !x.isValid(); });
      cDebug() << Logger::SubEntry << "got" << fstabEntries.count()
               << "fstab entries from" << lineCount << "lines in"
               << fstabFile.fileName();
    } else {
...
static QString findPartitionPathForMountPoint(const FstabEntryList &fstab,
    ...
    if (entry.mountPoint == mountPoint) {
      ...
      if (partPath.startsWith("/dev/disk/by-")) // we got a fancy node
      {
        ...
        if (readlink.exitCode() != 0 ||
            readlink.exitStatus() != QProcess::NormalExit) {
          return QString();
        }
        partPath =
            QString::fromLocal8Bit(readlink.readAllStandardOutput()).trimmed();
      }
...
OsproberEntryList runOsprober(DeviceModel *dm) {
  ...
  if (!osprober.waitForStarted()) {
    ...
    osproberOutput.append(
        QString::fromLocal8Bit(osprober.readAllStandardOutput()).trimmed());
  }
  ...
  for (const QString &line : lines) {
    if (!line.simplified().isEmpty()) {
      ...
      osproberEntries.append({prettyName, path, file, QString(),
                              canBeResized(dm, path, o), lineColumns,
                              fstabEntries, homePath});
      osproberCleanLines.append(line);
    }
  }

  if (osproberCleanLines.count() > 0) {
    cDebug() << o << "os-prober lines after cleanup:"
             << Logger::DebugList(osproberCleanLines);
  } else {
    cDebug() << o << "os-prober gave no output.";
  }

  Calamares::JobQueue::instance()->globalStorage()->insert("osproberLines",
                                                           osproberCleanLines);
...
bool isEfiBootable(const Partition *candidate) {
  ...
  static_assert(KPM_PARTITION_FLAG_ESP == KPM_PARTITION_FLAG(Boot),
                "KPMCore API enum changed");
...
QString canonicalFilesystemName(const QString &fsName,
  ...
  QStringList fsLanguage{
      QLatin1String("C")}; // Required language list to turn off localization

  if ((type = FileSystem::typeForName(fsName, fsLanguage)) !=
      FileSystem::Unknown) {
    return fsName;
  }
  ...
  for (FileSystem::Type t : fstypes) {
    if (0 == QString::compare(fsName, FileSystem::nameForType(t, fsLanguage),
                              Qt::CaseInsensitive)) {
...
FstabEntry FstabEntry::fromEtcFstab(const QString &rawLine) {
